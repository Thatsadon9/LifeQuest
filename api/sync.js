...
var STAT_KEYS = STATS.map((s) => s.key);
var STAT_BY_KEY = STATS.reduce(
  (acc, s) => {
    acc[s.key] = s;
    return acc;
  },
...
function nextStatLevelXP(level) {
  return 50 + level * 30;
}
function statLevelFromXP(xp) {
  let level = 1;
...
function mergeQuests(local, remote) {
...
function mergeCompletions(local, remote) {
  const key = (c) => `${c.questId}\0${c.date}`;
  const map = /* @__PURE__ */ new Map();
...
function mergeSkillNodes(local, remote) {
  const map = /* @__PURE__ */ new Map();
  ...
  for (const r of remote) {
    ...
    map.set(
      r.id,
      rankB >= rankA ? { ...r, title: rankB > rankA ? r.title : existing.title } : existing
    );
  }
  return [...map.values()];
}
function mergeEnergyCheckins(local, remote) {
  const map = /* @__PURE__ */ new Map();
...
function applyDeletions(bundle, deletions) {
...
function mergeBundles(local, remote, deletions) {
  ...
  const profile = localProfile && remoteProfile ? [mergeProfile(localProfile, remoteProfile)] : localProfile ? a.profile : b.profile;
  return {
...
function rowsToBundle(rows) {
  const hasAny = rows.profile.length + rows.stats.length + rows.quests.length + rows.completions.length + rows.skillNodes.length + rows.energyCheckins.length + rows.reviews.length > 0;
...
function rowToProfile(row) {
  return {
    ...
    updatedAt: row.updated_at != null ? num(row.updated_at) : void 0
  };
}
...
function rowToQuest(row) {
  return {
    ...
    createdAt: num(row.created_at),
    category: row.category ? str(row.category) : void 0,
    trackMode: row.track_mode ? str(row.track_mode) : void 0,
    targetCount: row.target_count != null ? num(row.target_count) : void 0,
    unit: row.unit ? str(row.unit) : void 0,
    updatedAt: row.updated_at != null ? num(row.updated_at) : void 0
  };
}
function rowToCompletion(row) {
  return {
    ...
    date: str(row.date),
    completionType: str(row.completion_type),
    xpAwarded: num(row.xp_awarded),
    statGains: json(row.stat_gains, {}),
    progress: row.progress != null ? num(row.progress) : void 0,
    createdAt: num(row.created_at)
  };
}
function rowToSkillNode(row) {
  return {
    id: str(row.id),
...
function rowToEnergyCheckin(row) {
  return {
    date: str(row.date),
    value: num(row.value)
  };
...
async function loadBundle(sql, userId) {
  const [profile, stats, quests, completions, skillNodes, energyCheckins, reviews] = await Promise.all([
...
// server/mira/tools.ts
var statKey = {
  type: SchemaType.STRING,
  description: "Stat key: INT, FOCUS, STR, WIS, CHA, CRAFT, or COIN"
};
var MIRA_FUNCTION_DECLARATIONS = [
  {
    name: "get_today_summary",
    description: "Get detailed summary of today: habits, completions, XP earned today, pending items.",
...
// server/mira/system.ts
function thaiLanguageRules() {
...
function buildMiraSystemInstruction(context, locale) {
  const lang = locale === "th" ? "Thai" : "English";
  const languageRules = locale === "th" ? thaiLanguageRules() : englishLanguageRules();
  const habits = context.habitsToday.map(
    (h) => `- ${h.title} (id:${h.id}) ${h.completed ? "DONE" : h.skipped ? "SKIPPED" : "pending"}${h.trackMode === "counter" ? ` ${h.progress ?? 0}/${h.targetCount}` : ""}`
  ).join("\n");
  return `# Identity
  ...
  for (const turn of turns) {
    if (turn.role === "user") {
      ...
      if (turn.functionResults?.length) {
        const parts = turn.functionResults.map((fr) => {
...
async function handleMiraChat(body) {
  ...
  try {
    ...
    return { text: text || "", modelParts: modelParts.length ? modelParts : void 0 };
  } catch (err) {
...
var SESSION_COOKIE = "lq_session";
var SESSION_DAYS = Number(process.env.SESSION_DAYS ?? 30);
...
function sessionExpiryMs() {
  return Date.now() + SESSION_DAYS * 24 * 60 * 60 * 1e3;
}
async function createSession(pool2, userId) {
...
async function deleteSession(pool2, token) {
  await pool2.query(`DELETE FROM sessions WHERE token_hash = $1`, [
    hashSessionToken(token)
  ]);
}
async function deleteExpiredSessions(pool2) {
  await pool2.query(`DELETE FROM sessions WHERE expires_at < $1`, [Date.now()]);
}
async function findUserBySessionToken(pool2, token) {
...
function requireUser(pool2) {
  return async (c, next) => {
    const token = getCookie(c, SESSION_COOKIE);
    if (!token) {
      return c.json({ error: "Unauthorized", code: "no_session" }, 401);
    }
    const user = await findUserBySessionToken(pool2, token);
    if (!user) {
...
async function findUserByEmail(pool2, email) {
  const result = await pool2.query(
...
async function createUser(pool2, email, password, displayName) {
...
function setSessionCookie(c, token) {
  setCookie(c, SESSION_COOKIE, token, {
    ...
    sameSite: "Lax",
    path: "/",
    maxAge: Math.floor((sessionExpiryMs() - Date.now()) / 1e3)
  });
}
function clearSessionCookie(c) {
  deleteCookie(c, SESSION_COOKIE, { path: "/" });
}
function registerAuthRoutes(app, pool2) {
  app.post("/api/auth/register", rateLimit(8, 6e4), async (c) => {
    let body;
    try {
    ...
    const existing = await findUserByEmail(pool2, email);
    if (existing) {
      return c.json({ error: "An account with this email already exists" }, 409);
    }
    try {
      const user = await createUser(pool2, email, password, displayName);
      const { token } = await createSession(pool2, user.id);
  ...
  app.post("/api/auth/login", rateLimit(12, 6e4), async (c) => {
    let body;
    try {
      body = await c.req.json();
    } catch {
    ...
    const user = await findUserByEmail(pool2, email);
  ...
  app.post("/api/auth/logout", async (c) => {
    const token = getCookie(c, SESSION_COOKIE);
    if (token) {
      await deleteSession(pool2, token);
    }
...
function getAllowedOrigins() {
  ...
  return [.../* @__PURE__ */ new Set([...defaults, ...fromEnv, ...vercel])];
}

// server/app.ts
function createApp(pool2) {
  const app = new Hono2();
  ...
  registerAuthRoutes(app, pool2);
  app.get(
    "/api/health",
    (c) => c.json({ ok: true, service: "lifequest-api", auth: true })
  );
  ...
  app.post(
    ...
    async (c) => {
      ...
      try {
        ...
        const toSave = { ...merged, exportedAt };
        await saveBundle(tx, userId, toSave);
        await client.query("COMMIT");
        const response = { bundle: toSave, exportedAt };
  ...
  void deleteExpiredSessions(pool2).catch(() => {
  });
  return app;
}
...
function configureNeonForRuntime() {
  if (isVercelRuntime()) {
...
// scripts/vercel-sync-entry.ts
var boot = (() => {
  try {
    return { app: createApp(getPool()), error: null };
  } catch (err) {
    const message = err instanceof Error ? err.message : String(err);
...
var vercel_sync_entry_default = handler;
var config = {
  runtime: "nodejs",
  maxDuration: 30
};
export {
  config,
  vercel_sync_entry_default as default
};
